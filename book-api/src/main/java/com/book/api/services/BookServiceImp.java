...
public class BookServiceImp implements BookService {
    ...
    public BookDto addBook(BookDto bookDto, MultipartFile file) throws IOException {
        if (Files.exists(Paths.get(path + File.separator + file.getOriginalFilename()))) {
            throw new FileAlreadyExistsException("File already exists! Please give anothe file.");
        }
    ...
    public BookDto updateBook(Long isbn, BookDto bookDto, MultipartFile file) throws IOException {
        ...
        if (file != null && book.getBookCover() != null) {
            String oldFile = book.getBookCover();

            bookCover = fileService.uploadFile(path, file);

            if (!oldFile.equals(bookCover)) {
                Files.deleteIfExists(Paths.get(path, oldFile));
            }
    ...
    public String deleteBook(Long isbn) throws IOException {
        ...
        if (book.getBookCover() != null) {
            Path filePath = Paths.get(path, book.getBookCover());
